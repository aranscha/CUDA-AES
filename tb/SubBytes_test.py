import numpy as np
import pycuda.driver as cuda
from pycuda.compiler import SourceModule
import pycuda.autoinit

class SubBytesTest:
    def __init__(self):
        self.getSourceModule()

        self.sbox = np.array([
            
        ], dtype=np.byte)

        self.sbox_inv = np.array([
            ,
        ], dtype=np.byte)

    def getSourceModule(self):
        with open("../kernels/SubBytes.cuh",  "r") as file:
            kernelWrapper = file.read()

        enable_test = r"""
        #define TEST_SUBBYTES
        """

        self.module = SourceModule(enable_test + kernelWrapper)

    def subbytes_gpu(self, message, length, inverse = False):
        # Event objects to mark the start and end points
        start = cuda.Event()
        end = cuda.Event()

        # Start recording execution time
        start.record()

        # Device memory allocation for input and output arrays
        io_message_gpu = cuda.mem_alloc_like(message)
        i_sbox_gpu = self.module.get_global('sbox')[0]

        # Copy data from host to device
        cuda.memcpy_htod(io_message_gpu, message)
        if inverse:
            cuda.memcpy_htod(i_sbox_gpu, self.sbox_inv)
        else:
            cuda.memcpy_htod(i_sbox_gpu, self.sbox)

        # Call the kernel function from the compiled module
        prg = self.module.get_function("SubByteTest")

        # Calculate block size and grid size
        block_size = length
        grid_size = 1
        if (block_size > 1024):
            block_size = 1024
            grid_size = (length - 1) / 1024 + 1;

        blockDim = (block_size, 1, 1)
        gridDim = (grid_size, 1, 1)

        # Call the kernel loaded to the device
        prg(io_message_gpu, np.uint32(length), block=blockDim, grid=gridDim)

        # Copy result from device to the host
        res = np.empty_like(message)
        cuda.memcpy_dtoh(res, io_message_gpu)

        # Record execution time (including memory transfers)
        end.record()
        end.synchronize()

        # return a tuple of output of sine computation and time taken to execute the operation (in ms).
        return res, start.time_till(end) * 1e-3

def test1_subbytes():
    # input array
    hex_in = "00102030405060708090a0b0c0d0e0f0" 
    byte_in = bytes.fromhex(hex_in)
    byte_array_in = np.frombuffer(byte_in, dtype=np.byte)

    # reference input
    hex_ref = "63cab7040953d051cd60e0e7ba70e18c"
    byte_ref = bytes.fromhex(hex_ref)
    byte_array_ref = np.frombuffer(byte_ref, dtype=np.byte)
    
    graphicsComputer = SubBytesTest()
    result_gpu, _ = graphicsComputer.subbytes_gpu(byte_array_in, byte_array_in.size)

    assert np.array_equal(result_gpu, byte_array_ref)

def test2_subbytes():
    # input array
    hex_in = "89d810e8855ace682d1843d8cb128fe4" 
    byte_in = bytes.fromhex(hex_in)
    byte_array_in = np.frombuffer(byte_in, dtype=np.byte)

    # reference input
    hex_ref = "a761ca9b97be8b45d8ad1a611fc97369"
    byte_ref = bytes.fromhex(hex_ref)
    byte_array_ref = np.frombuffer(byte_ref, dtype=np.byte)

    graphicsComputer = SubBytesTest()
    result_gpu, _ = graphicsComputer.subbytes_gpu(byte_array_in, byte_array_in.size)

    assert np.array_equal(result_gpu, byte_array_ref)

# test inverse subbytes
def test3_subbytes():
    # input array
    hex_in = "a761ca9b97be8b45d8ad1a611fc97369" 
    byte_in = bytes.fromhex(hex_in)
    byte_array_in = np.frombuffer(byte_in, dtype=np.byte)

    # reference input
    hex_ref = "89d810e8855ace682d1843d8cb128fe4"
    byte_ref = bytes.fromhex(hex_ref)
    byte_array_ref = np.frombuffer(byte_ref, dtype=np.byte)

    graphicsComputer = SubBytesTest()
    result_gpu, _ = graphicsComputer.subbytes_gpu(byte_array_in, byte_array_in.size, inverse=True)

    assert np.array_equal(result_gpu, byte_array_ref)

def test4_subbytes():
    # input array
    hex_in = "63cab7040953d051cd60e0e7ba70e18c" 
    byte_in = bytes.fromhex(hex_in)
    byte_array_in = np.frombuffer(byte_in, dtype=np.byte)

    # reference input
    hex_ref = "00102030405060708090a0b0c0d0e0f0"
    byte_ref = bytes.fromhex(hex_ref)
    byte_array_ref = np.frombuffer(byte_ref, dtype=np.byte)
    
    graphicsComputer = SubBytesTest()
    result_gpu, _ = graphicsComputer.subbytes_gpu(byte_array_in, byte_array_in.size, inverse=True)

    assert np.array_equal(result_gpu, byte_array_ref)
